...
Original file is located at
    https://colab.research.google.com/drive/1B3LsXplRTi_KqGetD8CeTzhW3Qcvt1Zq
"""
...
# 0. Kill all existing ngrok tunnels and clear the port
print("Killing all existing ngrok tunnels...")
ngrok.kill() # This closes all active tunnels and ngrok processes

print(f"Checking for and killing any process on port {STREAMLIT_PORT}...")
try:
    # Use fuser to find and kill the process using the port
    subprocess.run(
...
# 2. Start Streamlit in the background and capture output
...
# Use PIPE to capture stdout/stderr for better error diagnosis
...
# 3. Check if Streamlit is still running
if streamlit_process.poll() is not None:
    ...
    # 4. Create a public tunnel only if Streamlit is running
    try:
        ...
        # 5. Keep the cell alive to keep the Streamlit process and tunnel open
        while True:
            # Check if the Streamlit process has terminated unexpectedly
            if streamlit_process.poll() is not None:
        ...
        # Clean up the Streamlit process if ngrok fails
        streamlit_process.terminate()
